 ...
 * Text on fields
 */
const char* LOGOS_NAMES[] = {"HIGH SCORE", "SCORE",       "LEVEL",
...
void paint_boxes(void) {
  ...
  mvprintw(getmaxy(frame) + 2, main_begin_x,
           "P - pause, Q - end game, SPACE - action\n"
           "        ARROWS - moving");
}
...
void print_game_over(GameInfo_t info) {
  ...
  mvprintw(7, 5, "    YOUR SCORE: %d", info.score);
...
void print_terminate(GameInfo_t info) {
  ...
  mvprintw(6, 5, "YOUR SCORE: %d", info.score);
}
    ...
    for (int j = 0; j < COLS; j++) {
      mvwaddch(main_field, i, j * 2,
               (info.field[i][j] == CURRENT_FIGURE_BLOCK)
                   ? L_BLOCK
                   : ((info.field[i][j] == ATTACHED_BLOCK) ? L_ATTACHED : ' '));
      mvwaddch(main_field, i, j * 2 + 1,
               (info.field[i][j] == CURRENT_FIGURE_BLOCK)
                   ? R_BLOCK
                   : ((info.field[i][j] == ATTACHED_BLOCK) ? R_ATTACHED : ' '));
    }
  }
...
void fill_next_figure(GameInfo_t info) {
  // FULL_INFO;
  for (int field_i = 2, fig_i = 1; fig_i < MAX_FIG_ROWS; field_i++, fig_i++) {
    for (int field_j = 3, fig_j = 0; fig_j < MAX_FIG_COLS; field_j++, fig_j++) {
      mvwaddch(
          next_fig_field, field_i, field_j * 2,
          (info.next[fig_i][fig_j] == CURRENT_FIGURE_BLOCK) ? L_ATTACHED : ' ');
      mvwaddch(
          next_fig_field, field_i, field_j * 2 + 1,
          (info.next[fig_i][fig_j] == CURRENT_FIGURE_BLOCK) ? R_ATTACHED : ' ');
    }
  }
...
/* void fill_next_figure(GameInfo_t info) {
  ...
  for (int i = 0; i < MAX_FIG_ROWS; i++) {
    for (int j = 0; j < MAX_FIG_COLS; j++) {
      mvwaddch(next_fig_field, i + 2, j * 2 + 1,
               (info.next[i][j] == CURRENT_FIGURE_BLOCK)
                   ? L_ATTACHED : ' ');
      mvwaddch(next_fig_field, i + 2, j * 2 + 2,
               (info.next[i][j] == CURRENT_FIGURE_BLOCK)
                   ? R_ATTACHED
                   : ' ');
    }
